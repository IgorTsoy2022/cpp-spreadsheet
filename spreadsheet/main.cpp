...
namespace {
    ...
    void TestPositionAndStringConversion() {
        ...
        for (int i = 0; i < 25; ++i) {
            testSingle(Position{ i, i }, 
                       char('A' + i) + std::to_string(i + 1));
        }
    ...
    void TestStringToPositionInvalid() {
        ...
        ASSERT(!Position::FromString("A1234567890123456789")
                          .IsValid());
        ASSERT(!Position::FromString("ABCDEFGHIJKLMNOPQRS8")
                          .IsValid());
    }
    ...
    void TestSetCellPlainText() {
        ...
        auto checkCell = [&](Position pos, std::string text) {
            ...
            ASSERT_EQUAL(std::get<std::string>(cell->GetValue()),
                         text);
        };
        ...
        ASSERT_EQUAL(constSheet.GetCell("B2"_pos)->GetText(),
                     "Purr");
    ...
    void TestFormulaArithmetic() {
        ...
        ASSERT_EQUAL(evaluate("(12+13) * (14+(13-24/(1+1))*55-46)"),
                     575);
    }
    ...
    void TestFormulaReferencedCells() {
        ...
        ASSERT_EQUAL(a1->GetReferencedCells(),
                     (std::vector{ "A1"_pos }));

        auto b2c3 = ParseFormula("B2+C3");
        ASSERT_EQUAL(b2c3->GetReferencedCells(),
                     (std::vector{ "B2"_pos, "C3"_pos }));
    ...
    void TestCellCircularReferences() {
        ...
        sheet->SetCell("B2"_pos, "=B4+A3");
        sheet->SetCell("B4"_pos, "=D7");
        sheet->SetCell("D7"_pos, "=E6");
        sheet->SetCell("E6"_pos, "Ready");

        bool caught = false;
        try {
            sheet->SetCell("E6"_pos, "=B2");
        }
        ...
        ASSERT(caught);
        ASSERT_EQUAL(sheet->GetCell("E6"_pos)->GetText(),
                     "Ready");
    }
}  // namespace
