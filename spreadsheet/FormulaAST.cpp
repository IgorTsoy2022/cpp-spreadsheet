...
namespace ASTImpl {
    ...
    // PRECEDENCE_RULES[parent][child] determines if parentheses
    // need to be inserted between a parent and a child of specific
    // precedences;
    // for some nodes rules are different for left and right
    // children:
    // (X c Y) p Z  vs  X p (Y c Z)
    //
    // The interesting cases are the ones where removing the parens
    // would change the AST.
    // It may happen when our precedence rules for parentheses
    // are different from the grammatic precedence of operations.
    //
    // Case analysis:
    // A + (B + C) - always okay (nothing of lower grammatic
    //               precedence could have been written to the
    //               right)
    //               (e.g. if we had A + (B + C) / D, it wouldn't
    //               parse in a way that woudld have given us
    //               A + (B + C) as a subexpression to deal with)
    // A + (B - C) - always okay (nothing of lower grammatic
    //               precedence could have been written to the
    //               right)
    // A - (B + C) - never okay
    // A - (B - C) - never okay
    // A * (B * C) - always okay (the parent has the highest
    //               grammatic precedence)
    // A * (B / C) - always okay (the parent has the highest
    //               grammatic precedence)
    // A / (B * C) - never okay
    // A / (B / C) - never okay
    // -(A + B)    - never okay
    // -(A - B)    - never okay
    // -(A * B)    - always okay (the resulting binary op has the
    //               highest grammatic precedence)
    // -(A / B)    - always okay (the resulting binary op has the
    //               highest grammatic precedence)
    // +(A + B)    - **sometimes okay** (e.g. parens in
    //               +(A + B) / C are **not** optional)
    //               (currently in the table we're always putting
    //               in the parentheses)
    ...
    constexpr PrecedenceRule PRECEDENCE_RULES[EP_END][EP_END] = {
        /* EP_ADD */   { PR_NONE, PR_NONE, PR_NONE,
                         PR_NONE, PR_NONE, PR_NONE },
        /* EP_SUB */   { PR_RIGHT, PR_RIGHT, PR_NONE,
                         PR_NONE, PR_NONE, PR_NONE },
        /* EP_MUL */   { PR_BOTH, PR_BOTH, PR_NONE,
                         PR_NONE, PR_NONE, PR_NONE },
        /* EP_DIV */   { PR_BOTH, PR_BOTH, PR_RIGHT,
                         PR_NONE, PR_RIGHT, PR_NONE },
        /* EP_UNARY */ { PR_BOTH, PR_BOTH, PR_NONE,
                         PR_NONE, PR_NONE, PR_NONE },
        /* EP_ATOM */  { PR_NONE, PR_NONE, PR_NONE,
                         PR_NONE, PR_NONE, PR_NONE },
    };
    ...
    public:
        ...
        virtual double Evaluate(std::function<double(Position)> cell) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            void DoPrintFormula(std::ostream& out,
                ...
                out << static_cast<char>(type_);
                rhs_->PrintFormula(out, precedence,
                                   /* right_child = */ true);
            }
            ...
            double Evaluate(std::function<double(Position)>
                            cell) const override {
                auto lhs = lhs_->Evaluate(cell);
                auto rhs = rhs_->Evaluate(cell);
        ...
        public:
            explicit UnaryOpExpr(Type type,
        ...
        public:
            void exitUnaryOp(FormulaParser::UnaryOpContext*
                             ctx) override {
                assert(args_.size() >= 1);
                ...
                auto node = std::make_unique<UnaryOpExpr>(type,
                    std::move(operand));
                args_.back() = std::move(node);
            }

            void exitLiteral(FormulaParser::LiteralContext*
                             ctx) override {
                double value = 0;
                auto valueStr = 
            ...
            void exitCell(FormulaParser::CellContext*
                          ctx) override {
                auto value_str = 
                    ctx->CELL()->getSymbol()->getText();
            ...
            void exitBinaryOp(FormulaParser::BinaryOpContext*
                              ctx) override {
                assert(args_.size() >= 2);
                ...
                auto node = std::make_unique<BinaryOpExpr>(type,
...
double FormulaAST::Execute(std::function<double(Position)>
                           cell) const {
    return root_expr_->Evaluate(cell);
}
